...
| --------------- | --------------------------------------------------------- | -------------------------------- |
| Skip            | State version, mod list, per-mod content fingerprints, game fingerprint, blocked WADs and string-override locales all match, and every recorded WAD exists | Nothing is written |
...
WAD v3.4 fixes the TOC directly after the 272-byte header and a `u32` chunk
count, but each TOC entry carries an absolute `data_offset`, so data placement is
free. A patched WAD uses that:

```text
[header 272 B][chunk count u32][TOC: 32 B x toc_capacity]
...
Passthrough TOC entries are the source entries with `data_offset` shifted by one
...
4. Every passthrough entry in the overlay's TOC equals the source entry shifted
