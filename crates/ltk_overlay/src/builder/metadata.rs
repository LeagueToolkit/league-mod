...
fn build_override_meta(source: OverrideSource, bytes: &[u8]) -> Result<(WadHash, OverrideMeta)> {
...
impl OverlayBuilder {
    ...
    pub(crate) fn collect_all_override_metadata(
        ...
        let mut per_mod_results: Vec<HashMap<WadHash, OverrideMeta>> =
    ...
    fn build_mod_wad_reports(
        &self,
        per_mod_results: &[HashMap<WadHash, OverrideMeta>],
        fingerprints: &[Option<u64>],
        game_index: &GameIndex,
    ) -> Vec<ModWadReport> {
        self.enabled_mods
...
mod tests {
    ...
    fn test_unknown_wad_uses_overlap_fallback() {
        ...
        let entry_b = &meta[&WadHash(0xBBBB)];
        assert_eq!(
    ...
    fn test_unknown_wad_no_overlap_sets_fallback_none() {
        ...
        assert!(
            meta[&WadHash(0xCCCC)].fallback_wad.is_none(),
    ...
    fn test_unroutable_override_routed_to_dominant_wad() {
        ...
        assert_eq!(
            meta[&WadHash(0xCCCC)].fallback_wad.as_deref(),
            Some(ahri),
    ...
    fn test_filter_rejects_mod_shipped_stringtable_chunks() {
        ...
        assert!(all_meta.contains_key(&WadHash(0xAAAA)));
    }
    ...
    fn test_reconstruct_from_cache() {
        ...
        assert!(meta[&WadHash(0xABCD)].fallback_wad.is_none());
        assert_eq!(
            meta[&WadHash(0x1234)].linked_bins,
            vec!["data/characters/test/test.bin".to_string()]
        );
        assert!(meta[&WadHash(0xABCD)].linked_bins.is_empty());
    }
