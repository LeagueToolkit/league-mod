...
pub struct LayerTransformPlan {
    ...
    pub excluded_inputs: HashSet<PathBuf>,
    ...
    pub expected_outputs: HashSet<PathBuf>,
}
...
pub fn plan_transforms(mod_project: &ModProject, content_dir: &Path) -> TransformPlan {
    ...
    for layer in &mod_project.layers {
        ...
        if let Ok(paths) = glob::glob(layer_dir.join("**/*").to_string_lossy().as_ref()) {
                ...
                for (name, patterns) in &compiled {
                    if patterns.iter().any(|p| p.matches(rel_str)) {
                        ...
                        if name == "tex-converter" {
                            layer_plan.excluded_inputs.insert(entry.clone());
